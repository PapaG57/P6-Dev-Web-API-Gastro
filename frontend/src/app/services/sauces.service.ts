...
export class SaucesService {
  ...
  private apiUrl = environment.apiUrl + '/sauces';

  constructor(private http: HttpClient,
              private auth: AuthService) {}

  getSauces() {
    this.http.get<Sauce[]>(this.apiUrl).pipe(
  ...
  getSauceById(id: string) {
    return this.http.get<Sauce>(this.apiUrl + '/' + id).pipe(
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      return this.http.put<{ message: string }>(this.apiUrl + '/' + id, sauce).pipe(
        catchError(error => throwError(error.error.message))
      );
    } else {
      const formData = new FormData();
